...
namespace IntegrationTests.Yaml
{
	public class YamlParserTest
	{
		...
		private static async Task ParseNodesWithCommentsShouldWorkProperly(List<IYamlNode> nodes)
		{
			...
			Assert.Equal("Comment 1", nodes[0].Children.ElementAt(0).LeadingComments[0].Value);
			Assert.Equal("Comment 2", nodes[0].Children.ElementAt(0).LeadingComments[1].Value);
