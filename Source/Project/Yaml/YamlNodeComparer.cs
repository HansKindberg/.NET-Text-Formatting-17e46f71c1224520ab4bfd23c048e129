...
namespace HansKindberg.Text.Formatting.Yaml
{
	...
	public class YamlNodeComparer(YamlFormatOptions options) : IComparer<IYamlNode>
	{
		...
		public virtual int Compare(IYamlNode x, IYamlNode y)
		{
			...
			//if(x.Parent == null) // Document sorting
			//{
			//	return this.Compare(x, y, this._options.DocumentSorting);
			...
			if(this.Options.ScalarSorting.Enabled)
			{
				return 12345;
				/*
					var comparison = 0; // string.Compare(first, second, StringComparison.Ordinal);

				   if(comparison == 0)
				    return 0;

				   if(this._options.Sorting.Direction == ListSortDirection.Ascending)
				    return comparison;
